...
class ContactForm(forms.Form):
    BU_NAMES = (
            (u'SRANSW3', u'SRAN SW3'),
            (u'TDLTE', u'TDLTE'),
            (u'MGWOSP', u'MGW & OSP'),
            (u'SRANBTS', u'SRAN BTS'),
            (u'LCVSP', u'LC VSP'),
            (u'LCBGW', u'LC BGW'),
            (u'SYSTEMMODULE', u'System Module'),
            (u'RNCIV', u'SRAN SyVe & RNC I&V'),
            (u'others', u'others'),
    ...
    co_author_info = forms.CharField(label='co-authors (if any, free text)')
    gitlab_repository = forms.URLField(label='gitlab Repository')
...
def _store_enrollment_info(authorForm):
    try:
        ...
        c.gitlab_repository = authorForm.cleaned_data['gitlab_repository']
        c.save()
        return True
...
def _post(request):
    ...
    repos_address = authorForm.cleaned_data['gitlab_repository']
    repos_address_notification = 'Here comes the url for gitlab repository, Please login with your nsn-intra name and password ' + repos_address
